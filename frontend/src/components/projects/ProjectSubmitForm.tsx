...
import { X, Send, Loader2, Plus, UserRound, Calendar, Sparkles, FileText, Trash2 } from "lucide-react"
import api from "@/lib/api"
import { useAuthStore } from "@/store/authStore"

const DRAFT_KEY = "matriz_okr_draft"

interface DraftData {
  title: string
  okrObjectives: string
  keyResults: string
  keyActions: string
  resources: string
  fiveWhys: string
...
export default function ProjectSubmitForm({ onClose, onSuccess }: Props) {
  ...
  const [okrObjectives, setOkrObjectives] = useState(draft?.okrObjectives ?? "")
  const [keyResults, setKeyResults] = useState(draft?.keyResults ?? "")
  const [keyActions, setKeyActions] = useState(draft?.keyActions ?? "")
  const [resources, setResources] = useState(draft?.resources ?? "")
  const [fiveWhys, setFiveWhys] = useState(draft?.fiveWhys ?? "")
  const [measurementMethods, setMeasurementMethods] = useState(draft?.measurementMethods ?? "")
  const [okrCreator, setOkrCreator] = useState(draft?.okrCreator ?? "")
  const [collaboratorInput, setCollaboratorInput] = useState("")
  const [collaborators, setCollaborators] = useState<string[]>(
    draft?.collaborators ?? [uploaderName]
  )
  ...
  useEffect(() => {
    ...
    saveTimerRef.current = setTimeout(() => {
      const data: DraftData = {
        title, okrObjectives, keyResults, keyActions,
        resources, fiveWhys, measurementMethods, okrCreator, collaborators,
      }
      // Solo guardar si hay algo escrito
      if (title || okrObjectives || keyResults || keyActions || resources || fiveWhys || measurementMethods || okrCreator) {
        saveDraft(data)
        setHasDraft(true)
      }
    }, 800)
  ...
  }, [title, okrObjectives, keyResults, keyActions, resources, fiveWhys, measurementMethods, okrCreator, collaborators])
  ...
  return (
    <motion.div
      ...
      // Sin onClick aquí — click fuera no cierra el modal
    >
      <motion.div
      ...
      >
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-700/50">
          <div className="flex items-center gap-3">
            ...
            {hasDraft && (
              <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-amber-500/10 border border-amber-500/25 text-[11px] text-amber-400">
